...
class ColumnData(BaseModel):
    ...
    def _generate_header(self) -> str:
        ...
        timestamp = time.strftime(f"{core_config.datetimefmt} \n", time.localtime())
        header = (
...
class MainWindow(FileDropMixin, LogWindowMixin, MMainWindow):
    ...
    def save_file(self, append: bool = False, dialog: bool = False) -> bool:
        ...
        if dialog or not self.last_filename:
            prefilled_file = (
                self.last_filename if self.last_filename != "" else core_config.usersfolder
            )
            if append:
    ...
    def load_file(self) -> None:
        ...
        if self.dirty and not self.in_pytest and not save_messagebox(self, self.save_file):
            return
        prefilled_file = (
            self.last_filename if self.last_filename is not None else core_config.usersfolder
        )
        filename = QFileDialog.getOpenFileName(
            self,
            "Select input file",
