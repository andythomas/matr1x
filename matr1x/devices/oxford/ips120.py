...
class IPS120(IsobusDevice):
    ...
    def __init__(self, interface, isobus_addr=None, field_lim=0, **kwargs):
        ...
        field_lim : float, optional
            Maximum magnetic field limit in Tesla
        **kwargs : dict
    ...
    def getMagneticFields(self, setp=False):
        ...
        ----------
        setp : bool, optional
            If True, also returns the setpoint values

        Returns
        -------
        list or tuple
            If setp is False, returns a list containing the measured field value
            If setp is True, returns a tuple of two lists ([measured], [setpoint])
        """
    ...
    def setMagneticFieldRate(self, rate, axis):
        ...
        ----------
        rate : float
            Field ramp rate in T/min (can be between 0 and 0.5T/min)
        axis : int
            Axis identifier (0=x, -1=all axes)
        """
        if 0 > rate:
            rate = 0
        elif 0.5 < rate:
    ...
    def getMagneticFieldRate(self, axis, setp=False):
        ...
        -------
        float, list, or tuple
            Field rate in T/min. Format depends on axis and setp parameters.
        """
        val = None
        if -1 == axis:
    ...
    def setMagnetStatus(self, state, axis):
        ...
        axis : int
            Axis to control:
                -1 - takes state as list for all axes
                0 - x axis
                2 - z axis
        """
        try:
